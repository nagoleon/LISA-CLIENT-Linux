...
class keyword_spotting(object):
	...
	self.pipeline = gst.parse_launch('autoaudiosrc ! queue ! audioconvert ! audioresample ! audio/x-raw-int, rate=16000, width=16, depth=16, channels=1 ! tee name=t ! queue ! audioresample ! audio/x-raw-int, rate=8000 ! vader name=vad auto-threshold=true ! pocketsphinx name=asr ! fakesink dump=1 t. ! valve drop=0 ! queue ! flacenc ! filesink location=%s async=0' % FLACFILE)
	...
	req = urllib2.Request('https://www.google.com/speech-api/v1/recognize?client=chromium&lang=fr-FR&maxresults=10', flacfile, {'Content-Type': 'audio/x-flac; rate=16000'})
	res = urllib2.urlopen(req)

	resp = res.read()
	resp = json.loads(resp)
	print("structure: {}".format(resp))
